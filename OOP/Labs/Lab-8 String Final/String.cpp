...
{
	char ch;
	int index = s.getLenght();
	r.get(ch);
	while (ch != '\n' && !r.eof())
	{
		if (index == s.size)
		{
			s.reSize((s.size == 0 ? 1 : s.size) * 2);
		}
		s[index] = (char)ch;
		index = index + 1;
		r.get(ch);
...
{
	String decimal;
	long long int decimalPart = num;
	decimal = decimalPart;
	decimal += '.';
	double point = num - decimalPart;
	long long int  factionPart = point * 100000000;
	String faction;
	int i = 0;
	faction = factionPart;
	while (point < 0.1)
...
String String::operator ()(const String& delim)
{
	...
	if (isEmpty())
		return temp;
	int i = 0;
	while (!isDelimeter(delim, data[tokenIndex]))
	{
		temp.reSize(temp.getSize() + 1);
